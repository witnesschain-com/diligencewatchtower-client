...
func ValidateWatchtowerAddress(simplifiedConfig *SimplifiedConfig) bool {
	...
	if err != nil {
		...
		Error("Watchtower " + simplifiedConfig.WatchtowerAddress.Hex() + " address is not registered with WitnessChain, please ensure that your watchtower's eth address is registered with WitnessChain")
		return false
	}
